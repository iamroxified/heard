...
<section class="py-24 bg-slate-50 border-t border-slate-100 overflow-hidden">
  <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
    <div class="text-center max-w-2xl mx-auto mb-14" data-aos="fade-up">
      <span class="text-gold text-xs font-bold tracking-wider mb-3 block uppercase">In Action</span>
      <h2 class="text-3xl md:text-4xl font-heading font-bold text-dark">
        Featured Videos
      </h2>
      <p class="text-slate-500 text-sm mt-3 leading-relaxed">Watch African voices taking the world's most important stages.</p>
    </div>

    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
      @foreach ($featuredVideos as $fv)
      <a href="{{ $fv->watch_url }}" target="_blank" rel="noopener noreferrer"
        class="group block bg-white border border-slate-100 shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden"
        data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">

        <!-- Thumbnail with play overlay -->
        <div class="relative overflow-hidden aspect-video bg-slate-200">
          <img
            src="{{ $fv->thumbnail_url }}"
            alt="{{ $fv->title }}"
            class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
            onerror="this.src='https://img.youtube.com/vi/{{ $fv->youtube_id }}/hqdefault.jpg'">

          <!-- Play button overlay -->
          <div class="absolute inset-0 flex items-center justify-center bg-dark/30 group-hover:bg-dark/50 transition-colors duration-300">
            <div class="w-14 h-14 rounded-full bg-white/90 group-hover:bg-gold flex items-center justify-center shadow-lg transition-colors duration-300">
              <svg class="w-6 h-6 text-dark ml-1" fill="currentColor" viewBox="0 0 24 24">
          ...
          <!-- YouTube badge -->
          <div class="absolute top-3 right-3 bg-red-600 text-white text-xs font-bold px-2 py-1 flex items-center gap-1">
        ...
        <!-- Card body -->
        <div class="p-5">
          <h3 class="font-heading font-bold text-dark text-sm leading-snug group-hover:text-gold transition-colors duration-200">
            {{ $fv->title }}
          </h3>
          @if ($fv->description)
          <p class="text-slate-500 text-xs leading-relaxed mt-2 line-clamp-2">{{ $fv->description }}</p>
          @endif
          <span class="mt-4 inline-flex items-center gap-1 text-gold text-xs font-bold uppercase tracking-wider">
            Watch on YouTube
